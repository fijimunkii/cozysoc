...
test("AdGuard Home query history needs a separate bounded browser review", async ({ page }) => {
    ...
    switch (path) {
      ...
      case "/api/adguard/collection/review":
        ...
        return reply({ review_id: "r".repeat(43), expires_at: new Date(Date.now() + 5 * 60_000).toISOString(),
          endpoint: "https://192.0.2.5:3000", scope_id: "scope.home", interface: { interface_name: "en0", interface_index: 7,
            prefixes: ["192.0.2.0/24"] }, max_queries: 100, max_query_age_hours: 24 });
      case "/api/adguard/collection/run": {
        runs++;
        const body = route.request().postDataJSON();
        expect(body.review_id).toBe("r".repeat(43));
        expect(route.request().headers()["x-cozy-csrf"]).toBe("c".repeat(43));
        return reply(body.approve ? { outcome: "completed", result: { scope_id: "scope.home", query_log_enabled: true,
          read: 2, inserted: 1, deduplicated: 0, skipped_outside_scope: 1, skipped_without_client_ip: 0,
          skipped_outside_window: 0, limit_reached: false } } : { outcome: "declined" });
      }
  ...
  await expect(page.getByText("No DNS history read was approved.")).toBeVisible();
  expect(runs).toBe(1);
  await page.getByRole("button", { name: "Review one DNS history collection" }).click();
  await page.getByRole("button", { name: "Approve one read" }).click();
  await expect(page.getByText(/Read 2 recent queries; saved 1/)).toBeVisible();
