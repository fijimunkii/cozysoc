...
export function DiagnosticsPanel({ mode, load = loadDiagnosticPreview }: DiagnosticsPanelProps) {
  ...
  return <section className="product-card" aria-labelledby="diagnostics-title">
    ...
    <p>Review a small versioned support snapshot before saving it. It contains the controller build, curated adapter states, controller gaps, a category for Device Watch coverage failures, and separate database-quota and host-volume health states. Adapter versions describe Cozy SOC code, not external service versions. It excludes storage sizes, network names, addresses, URLs, paths, credentials, and raw errors. Nothing is uploaded.</p>
