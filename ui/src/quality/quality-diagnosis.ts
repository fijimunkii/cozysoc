...
export type DiagnosisConclusion = "not-enrolled" | "history-incomplete" | "insufficient-evidence" | "latest-run-unmeasured" | "observation-context-mismatch" | "observations-too-far-apart" | "dns-query-issue-with-responses" | "icmp-misses-with-responses" | "problems-across-selected-layers" | "selected-checks-matched" | "mixed-or-limited-evidence";
export interface DiagnosisReference { kind: "gateway" | "resolver"; run_id: string }
...
function runReference(raw: unknown): DiagnosisReference {
  ...
  return { kind: choice(v.kind, ["gateway", "resolver"]), run_id: v.run_id };
}
export function parseQualityDiagnosis(raw: unknown): QualityDiagnosis {
  ...
  const out: QualityDiagnosis = { enrolled: bool(v.enrolled), read_at: timestamp(v.read_at), since: timestamp(v.since), truncated: bool(v.truncated), scan_truncated: bool(v.scan_truncated), selected: [], compared: [],
    conclusion: choice(v.conclusion, ["not-enrolled", "history-incomplete", "insufficient-evidence", "latest-run-unmeasured", "observation-context-mismatch", "observations-too-far-apart", "dns-query-issue-with-responses", "icmp-misses-with-responses", "problems-across-selected-layers", "selected-checks-matched", "mixed-or-limited-evidence"]), confidence: choice(v.confidence, ["unknown", "limited"]) };
  for (const key of ["assessment_at", "evidence_start", "evidence_end"] as const) if (Object.hasOwn(v, key)) out[key] = timestamp(v[key]);
  if (nanos(out.read_at) - nanos(out.since) !== 86400000000000n || !Array.isArray(v.selected) || v.selected.length > 2 || !Array.isArray(v.compared) || v.compared.length > 2 ||
  ...
  for (const rawRun of v.selected) {
    const r = object(rawRun, ["kind", "run_id", "interface_name", "interface_index", "last_audit_at", "execution_outcome", "sample_status"], ["started_at", "completed_at", "selection"]);
    ...
    if (run.sample_status === "missing-terminal" || run.sample_status === "no-measurement") {
      ...
      if (run.completed_at !== undefined && (nanos(run.completed_at) < nanos(run.started_at) || nanos(run.completed_at) > nanos(run.last_audit_at) || nanos(run.completed_at) - nanos(run.started_at) >= 5000000000n)) return invalid();
      if ((run.sample_status === "recorded" || run.kind === "resolver") && run.completed_at === undefined) return invalid();
  ...
  const g = selected.get("gateway"), d = selected.get("resolver");
  ...
  else if (g!.sample_status !== "recorded" || d!.sample_status !== "recorded") expected = "latest-run-unmeasured";
  else if (g!.interface_name !== d!.interface_name || g!.interface_index !== d!.interface_index || d!.selection!.family !== "ipv4") expected = "observation-context-mismatch";
  else if (nanos(anchor!) - nanos(start!) > 86400000000000n) expected = "observations-too-far-apart";
  else if (nanos(anchor!) - nanos(g!.completed_at!) >= 30000000000n || nanos(anchor!) - nanos(d!.completed_at!) >= 30000000000n) expected = "insufficient-evidence";
  if (expected !== undefined) {
    ...
    if (!["dns-query-issue-with-responses", "icmp-misses-with-responses", "problems-across-selected-layers", "selected-checks-matched", "mixed-or-limited-evidence"].includes(out.conclusion) || out.confidence !== "limited" || v.compared.length !== 2 || out.evidence_start === undefined || out.evidence_end === undefined || nanos(out.evidence_start) !== nanos(start!) || nanos(out.evidence_end) !== nanos(end!)) return invalid();
