...
The process E2E tests live under `tests/e2e`. Ordinary `go test ./...` skips them unless `COZYSOC_E2E_BINARY` points to a built controller binary. CI builds and verifies the unsigned developer bundle with `scripts/build-dev-bundle.sh` and `scripts/dev-bundle-checksums.py`, then runs the E2E package against its executable and UI assets so the black-box step is visible as its own gate. Metadata regression checks require installed runtime packages and top-level license texts, select browser production dependencies without build tools, and parse the generated CycloneDX JSON. The checksum regression checks cover changed, missing, extra, symlinked, and corrupt-manifest cases. The web process test also launches a relocated copy without `--ui-dir` from an unrelated working directory and checks session/credential separation and controller survival when web exits.
