...
After a successful live read, the user can review and save version 1 `cozysoc-gateway-history` JSON for exactly that browser snapshot. Export revalidates the bounded projection and preserves the original window, read time, run references and both truncation flags. The file can contain private target/source IPv4 addresses and interface details. Saving the preview is local and does not read the controller or run a check. A refreshed list requires a new preview. This is neither complete history nor a backup; demo mode has no export.
