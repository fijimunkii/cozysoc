...
The authenticated Tools storage section reads the controller-owned store through `storage.overview` and shows active expiry durations, live page accounting, and a stored-record inventory. The inventory counts batch evidence records, separate observations and identity claims, coverage samples, findings, audits, selected-check settings (including retired records), and labeled device rows from one read-only database snapshot. Batch records may retain identity claims after an observation expires. Counts include rows awaiting expiry cleanup and are not a per-class byte breakdown, a count of currently visible devices, or a complete inventory of transient files, settings, Keychain secrets, user-saved exports, or connected services. SQLite page use remains the authoritative database-size measure. The view does not offer retention edits, deletion, backup, or restore; those #30 controls remain to be built. Its host-volume figure is filesystem capacity, not the size of Cozy SOC data outside the SQLite file.

Devices and Activity offer reviewed, local JSON exports of their current bounded read projections. The device export contains at most 100 retained identity records for one device; the Activity export contains the current 24-hour window. Each versioned file carries its original read time and truncation status, may contain household identifiers, and is neither a full history export nor a restorable backup. Saving does not query the controller again or upload data. See the [frontend export contract](frontend.md) for the exact behavior and privacy limits.

The Tools view also distinguishes expiring evidence from durable settings. Current Device Watch coverage samples use `short` retention; neighbor observations, IP/MAC identity claims, and informational first-arrival findings use `standard`; its collector does not use `ephemeral`. Separately approved AdGuard Home one-shot query observations use `ephemeral` (24 hours) and do not create device identity claims or coverage samples. Enrollment, label and identity-correction audits, plus reviewed network-check history, use `audit`. Those deadlines do not remove saved network scopes, sensor registrations, device rows/labels, identity corrections, selected-check settings, or controller configuration. User-saved JSON files are outside controller cleanup. The UI discloses these limits until #30 supplies deletion and recovery controls.
