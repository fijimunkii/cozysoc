...
preview includes a `route` review. Reading it cannot execute or grant consent.
The macOS adapter has synthetic coverage and
[isolated native runtime evidence](macos-network-lab.md); packaged permissions,
physical egress, VPN policy and hardware transitions remain unvalidated.
...
accepted by this preflight. The [native command](interactive-gateway-check.md)
and [browser review](browser-gateway-check.md) use the same read-only plan before
their separate one-shot approval boundaries.
...
The [ICMP sender](gateway-icmp-sender.md) validates source/interface/route and
socket binding immediately before every send; this preview DTO never becomes
send authority.
...
cancellation, and clock boundaries. Fuzz seeds run in normal Go tests. Controller
fixtures verify projection/minimization and that consistency never grants
authority. Linux process E2E verifies explicit unsupported route metadata while
previews leave monitoring off and history empty. The isolated macOS lab tests the
actual route adapter and sender against a controlled virtual peer, including
source loss. Packaged permission recovery, VPN/Network Extension policy,
physical egress, and sleep/resume remain separate release gates.
