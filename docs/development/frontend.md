...
- exposes only allowlisted browser routes: the one-time `POST /api/session` bootstrap, authenticated session/CSRF metadata, typed reads for status/capabilities/coverage/devices/activity/networks/network quality, narrow enrollment / Device Watch control mutations, and explicitly reviewed one-shot gateway, saved-resolver and saved-HTTPS checks;
