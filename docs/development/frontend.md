...
- exposes only allowlisted browser routes: the one-time `POST /api/session` bootstrap, authenticated session/CSRF metadata, typed reads for status/capabilities/coverage/devices/activity/networks/network quality, narrow enrollment / Device Watch and selected-target settings mutations, and explicitly reviewed one-shot gateway, saved-resolver and saved-HTTPS checks;
- exposes authenticated, parameterless `GET /api/adguard/status` only when the user requests a fresh external AdGuard Home status, including at most 32 reported blocklist and 32 allowlist sources with IDs, names, rule counts and optional update times. The browser receives no configured username, credential, query history, source URL, custom rule text or local controller secret;
- offers separate CSRF-protected `POST /api/adguard/collection/review` and `/run` actions from Tools. The short-lived, one-use review shows the external origin and enrolled interface/prefixes plus the 100-query and 24-hour eligibility bounds. Approval is rechecked against the same connection and scope in the controller before private history is read; responses contain counts only and uncertain outcomes require inspecting saved history before a new approval;
...
The [experimental browser resolver check](resolver-consent.md) uses an explicit saved-selection read and fixed review/run POST routes. The selection read is user-triggered and shows private endpoint/question settings only in the authenticated local UI. Review validates the saved selection, current source/interface/prefixes and complete DNS budget without sending traffic. A separate one-use approval invokes the connection-bound native consent session only if the fresh native review matches the displayed selection, DNS question, binding and budget. Native challenge and ticket never enter the browser. An uncertain response after approval is never retried automatically.

The [experimental browser HTTPS check](https-consent.md) likewise uses a user-triggered saved-selection read and fixed review/run routes. The review validates and displays the exact HTTP request bytes, TLS policy, numeric endpoint, source, route freshness, privacy text and all traffic ceilings. A one-use browser approval enters the native consent session only if the fresh review still matches the displayed execution details.

The candidate [OPNsense browser collection](opnsense-readonly.md) uses separate authenticated, CSRF-protected review/run routes. Review shows the saved private router origin and enrolled interface/prefixes without reading neighbor tables. Approval consumes a short-lived one-use review, then the controller revalidates that origin and scope and performs one audited, bounded ARP/NDP read. The browser receives counts only; retained router reports are read separately on demand. This does not establish router support or whole-network coverage.

The browser also has fixed `POST /api/network-quality/{resolver,https}/selections/save` and `/retire` routes backed by the existing native settings methods. Each save requires every supported target field and rejects duplicate, unknown or extra JSON. It stores private settings under the enrolled scope, with random immutable references; it does not inspect routes, send traffic, grant check consent or schedule a run. Retirement takes only a selected saved reference and requires a separate review in the UI. The browser response projects only the saved selection and settings, never scope or other controller internals. After an uncertain write response the UI requires a fresh selection read before another attempt. The check panels require a separate user-triggered selection load and one-shot review, so saving settings never starts a check. There is no generic RPC or arbitrary request builder.
...
Vite build output is not committed. `scripts/build-dev-bundle.sh DESTINATION` creates a relocatable unsigned developer directory with the unified executable, sibling `ui/dist/` assets, source/platform metadata, the first-party license, a CycloneDX inventory of the Go standard library, modules embedded in the executable and browser production packages, their top-level license/notice/patent texts, and a versioned `CHECKSUMS.json` inventory. Build-only npm tools are not shipped. The builder refuses an existing destination and verifies checksums before publishing the directory; `python3 scripts/dev-bundle-checksums.py verify DESTINATION` checks every expected file and rejects changes, omissions, extras, and symlinks. This detects accidental/tampered content against that local manifest, but the unsigned manifest cannot prove publisher identity. `cozysoc web` resolves the executable path (including executable symlinks) and prefers its sibling `ui/dist/` when a sibling `ui/` exists. An incomplete sibling UI fails startup; it does not silently serve an unrelated working-directory copy. Without sibling assets it uses the repository's `ui/dist` developer path. An explicit `--ui-dir` overrides either default. `cozysoc dev` continues to use the repository build path and its temporary-controller lifecycle.
...
- `GET /api/devices/detail?device_id=...` — bounded current-scope identity evidence and retained source provenance for one device, plus up to 100 recent AdGuard Home DNS observations associated only through unique, time-valid Device Watch IP evidence. Other raw observation payloads are not exposed. DNS names remain out of the identity-evidence JSON export.
...
The Findings page makes an on-demand authenticated read of at most 100 retained informational Device Watch first-arrival findings, newest by controller recording time. Initial and post-gap collections establish a baseline without arrival findings; subsequent newly observed identities during continuous collection appear here. It includes the original observation reference and marks whether that observation still has an unexpired storage reference at read time. The browser receives no raw finding payload, IP/MAC address, or notification authority. A local user can mark one retained arrival as reviewed through the authenticated, CSRF-protected route; the acknowledgement and audit event commit together, survive controller restart, and remain idempotent. This is review state only: it does not verify identity, suppress later findings, resolve an incident, or trigger a notification. A first-observed identity can reflect address rotation or a visibility gap; this view is not an incident verdict or complete history. A failed read stays visibly unavailable, and the synthetic demo has no findings.
...
The resolver-history section is a private, one-shot 24-hour view of retained observations, not a complete browsing history or network coverage. It omits unmatched, ambiguous, expired and incomplete identity associations and labels displayed matches as inferred. The read inspects at most 16 registered external AdGuard sensors and fails closed if that bound is exceeded.

The detail view offers an explicit, local JSON export for that one bounded read. The user reviews the exact versioned JSON before saving. The export serializes only the validated identity-evidence projection, so unexpected response fields and DNS query names cannot enter the file. It may contain household identifiers such as labels, IP/MAC addresses and source IDs; the UI says so before saving. It is limited to 100 retained identity records, carries the original read time and truncation flag, and is not a backup or complete history. It does not upload anything or read the controller again at save time. Full-history export and restore remain #30 work.
...
Version 3 contains the controller build/configuration schema, a bounded controller health state and gap count, desired/verification states for the curated Device Watch, AdGuard Home, and OPNsense adapters when present, a bounded Device Watch coverage state and failure category, and separate database-quota and host-volume states. Each adapter build version is Cozy SOC's controller build, not a claim about an external service's installed version. The storage read itself is marked `current` or `unavailable`; a failed read yields `unknown` capacity states without its error. Unsupported host-volume introspection remains distinct from full or available capacity. Unknown or malformed categories are reduced to `unknown` or rejected. The controller and web bridge do not copy storage sizes, capability descriptions, network/sensor/interface/device identifiers, settings, URLs, file paths, credentials, raw logs, database errors, or evidence payloads into the preview. A failed coverage read becomes a `read-failed` category without its raw error. This is a small support snapshot, not a log bundle or proof of complete monitoring; broader #30 export and recovery controls remain separate work.
