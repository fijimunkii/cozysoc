...
- exposes authenticated, parameterless `GET /api/adguard/status` only when the user requests a fresh external AdGuard Home status; the browser receives no configured username, credential, query history or local controller secret;
