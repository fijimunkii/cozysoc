...
Remaining work includes real DNS/router/traffic/wireless adapters and source-specific validation, permission evidence where a producer can actually prove it, multi-observation-point aggregation once a real capability has more than one point, named-workload calibration of operational thresholds, and real low-disk/full-volume/hardware evidence under #29. The frontend already renders the shared report contract; its multi-point path is exercised with synthetic fixtures only.
