...
| **DNS observation / filtering** | Candidate native connection to AdGuard Home v0.107.79 reads status; a separately approved one-shot read retains up to 100 scoped client-IP queries for 24 hours. The device detail shows only retained queries with a unique, recent, time-valid Device Watch IP association and marks that association as inferred. DNS activity is not counted as coverage. | Observed/managed resolver plus clients actually using it; signed Keychain runtime, device-history validation and coverage validation remain gates | Shared or reused addresses, encrypted DNS bypass, alternate resolvers, IPv6 configuration gaps, anonymized or unobserved clients, incomplete identity evidence and the 100-query read limit |
