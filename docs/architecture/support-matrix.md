...
| **DNS observation / filtering** | Candidate native connection to AdGuard Home v0.107.79 reads status; a separately approved one-shot read retains up to 100 scoped client-IP queries for 24 hours. No DNS activity is yet attributed to devices or counted as coverage. | Observed/managed resolver plus clients actually using it; signed Keychain runtime, identity/history UI and coverage validation remain gates | Encrypted DNS bypass, alternate resolvers, IPv6 configuration gaps, anonymized or unobserved clients, and the 100-query read limit |
