...
| **DNS observation / filtering** | Candidate native connection to AdGuard Home v0.107.79 reads status and bounded filter-source metadata, with a user-triggered browser view and validated external admin link. Source update times are resolver reported, not verified versions. A separately approved native or browser one-shot read considers up to 100 scoped client-IP queries from the last 24 hours; accepted evidence uses ephemeral retention (24 hours by default). Device detail shows only queries with a unique, recent, time-valid Device Watch IP association, clearly marked as inferred. DNS activity is not counted as coverage. | Observed/managed resolver plus clients actually using it; signed Keychain runtime, device-history validation and coverage validation remain gates | Shared or reused addresses, encrypted DNS bypass, alternate resolvers, IPv6 configuration gaps, anonymized or unobserved clients, incomplete identity evidence and the 100-query read limit |
