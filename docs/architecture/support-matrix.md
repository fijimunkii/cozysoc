...
| **DNS observation / filtering** | Candidate native connection to an existing AdGuard Home v0.107.79 instance reads service status only; no DNS activity is yet attributed to devices or counted as coverage | Observed/managed resolver such as AdGuard Home plus actual clients configured to use it; signed Keychain runtime and query ingestion validation remain gates | Encrypted DNS bypass, alternate resolvers, IPv6 configuration gaps, devices outside resolver path |
