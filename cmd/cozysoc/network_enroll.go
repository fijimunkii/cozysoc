...
func runNetworkRetireCommand(ctx context.Context, args []string, stdout, stderr *os.File) error {
	fs := flag.NewFlagSet("network-retire", flag.ContinueOnError)
	fs.SetOutput(stderr)
	stateDir := fs.String("state-dir", "", "controller state directory")
	if err := fs.Parse(args); err != nil {
		return err
	...
	if fs.NArg() != 1 {
		return fmt.Errorf("network-retire requires the reviewed SCOPE_ID; disable Device Watch first")
	}
	dir, err := resolveStateDir(*stateDir)
	if err != nil {
		return err
	}
	result, err := localapi.NewClient(dir).CallWithParams(ctx, api.MethodNetworkRetire, api.NetworkRetireParams{ScopeID: fs.Arg(0)})
	if err != nil {
		return err
	}
	var pretty api.NetworkRetireResult
	if err := json.Unmarshal(result, &pretty); err != nil {
		return fmt.Errorf("decode network retirement response: %w", err)
	}
	enc := json.NewEncoder(stdout)
	enc.SetIndent("", "  ")
	return enc.Encode(pretty)
