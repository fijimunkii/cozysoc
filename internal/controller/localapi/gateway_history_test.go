...
type historyTestHandler struct {
	testHandler
	calls   atomic.Int32
	bounded atomic.Bool
	err     error
...
func (h *historyTestHandler) GatewayHistory(ctx context.Context, p api.GatewayHistoryParams) (api.GatewayHistory, error) {
	h.calls.Add(1)
	_, ok := ctx.Deadline()
	h.bounded.Store(ok)
	return api.GatewayHistory{SchemaVersion: 1, Mode: "retained-history", AsOf: time.Now().UTC(), LookupRunID: p.RunID, Runs: []api.GatewayHistoryRun{}}, h.err
}

func TestGatewayHistoryAuthenticatedReadAndStrictParams(t *testing.T) {
	h := &historyTestHandler{}
	...
	for _, id := range []string{"", strings.Repeat("a", 32)} {
		...
		var err error
		if id == "" {
			raw, err = client.Call(context.Background(), api.MethodGatewayHistory)
		} else {
			raw, err = client.CallWithParams(context.Background(), api.MethodGatewayHistory, api.GatewayHistoryParams{RunID: id})
		}
		var result api.GatewayHistory
	...
	for _, params := range []string{`null`, `{}`, `{"run_id":null}`, `{"run_id":""}`, `{"Run_id":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"}`, `{"run_id":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","run_id":"bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"}`, `{"run_id":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","scope_id":"scope.other"}`, `{"target":"192.168.50.1"}`, `{"approve":true}`, `{"run_id":"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"}`} {
		_, err := client.CallWithParams(context.Background(), api.MethodGatewayHistory, json.RawMessage(params))
		var e *ResponseError
...
func TestHistoryCannotBypassPeerOrSecretChecks(t *testing.T) {
	for _, mode := range []string{"secret", "unverified", "wrong-uid"} {
		t.Run(mode, func(t *testing.T) {
			...
			_ = json.NewEncoder(conn).Encode(api.Request{Version: api.Version, ID: "history", Method: api.MethodGatewayHistory, Auth: auth})
...
func TestHistoryErrorsDoNotLeakRawAuditOrDatabaseContent(t *testing.T) {
	for _, cause := range []error{errors.New("private retained payload"), ErrReadTargetNotFound} {
		h := &historyTestHandler{err: cause}
		s := startMutationTestServer(t, h)
		_, err := NewClient(s.stateDir).Call(context.Background(), api.MethodGatewayHistory)
