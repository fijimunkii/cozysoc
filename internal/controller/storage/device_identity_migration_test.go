...
func TestDeviceIdentitySchemaV6MigratesAndRetriesAtomically(t *testing.T) {
	for _, conflict := range []bool{false, true} {
		t.Run(map[bool]string{false: "existing", true: "rollback"}[conflict], func(t *testing.T) {
			dir := t.TempDir()
			dsn, err := sqliteFileURI(filepath.Join(dir, Filename))
			...
			if _, err := db.ExecContext(context.Background(), migrationV1+migrationV2+migrationV3+migrationV4+migrationV5+`
				INSERT INTO network_scopes VALUES('scope.home','lan',1,NULL,'{}');
				INSERT INTO devices VALUES('device.existing',NULL,1,NULL);
			...
			if conflict {
				if _, err := db.Exec(`CREATE TABLE device_identity_splits(dummy INTEGER) STRICT`); err != nil {
					t.Fatal(err)
				}
			}
			if err := db.Close(); err != nil {
			...
			if conflict {
				if s, err := Open(dir, DefaultLimits()); err == nil {
					s.Close()
					t.Fatal("conflicting v6 object accepted")
				}
				db, err = sql.Open("sqlite", dsn)
				if err != nil {
					t.Fatal(err)
				...
				if err := db.QueryRow(`SELECT count(*) FROM sqlite_master WHERE name='device_identity_splits_source'`).Scan(&partial); err != nil {
					t.Fatal(err)
				}
				if version != 5 || partial != 0 {
					t.Fatal("failed migration changed v5", version, partial)
				}
				if _, err := db.Exec(`DROP TABLE device_identity_splits`); err != nil {
					t.Fatal(err)
				}
				if err := db.Close(); err != nil {
					t.Fatal(err)
			...
			var count int
			if err := s.conn.QueryRowContext(context.Background(), `SELECT count(*) FROM devices WHERE id='device.existing'`).Scan(&count); err != nil || count != 1 {
				t.Fatal("prior device changed", count, err)
			}
		})
	}
}
