...
var ErrHistory = errors.New("retained resolver evidence is invalid or unavailable")

// RetainedRun is historical evidence, never current connectivity or approval.
// Missing phases do not establish whether a query was or was not sent.
...
// normalized Selection/Observer/Reply field names. Unknown versions fail closed.
func DecodeRetainedEvent(raw []byte) (Event, error) {
	...
	if _, err = historyObject(fields["selection"], []string{"ID", "ResolverID", "QueryID", "Family", "Transport", "QueryType", "Expect"}, nil); err != nil {
		return Event{}, ErrHistory
	}
	...
	if raw, ok := fields["measurement"]; ok {
		m, err := historyObject(raw, []string{"started_at", "completed_at", "exchange", "request"}, []string{"gap", "reply", "response_time_ns"})
		if err != nil {
			return Event{}, ErrHistory
		}
		if reply, ok := m["reply"]; ok {
			if _, err = historyObject(reply, []string{"RCode", "Truncated", "Answer"}, nil); err != nil {
				return Event{}, ErrHistory
			}
		}
	}
...
func DescribeRetainedRun(events []Event, asOf time.Time) (RetainedRun, error) {
	...
	if out.Measurement == nil {
		out.Assessment.Summary = "The retained terminal audit contains no usable DNS measurement."
		return out, nil
	}
	...
	report, err := nq.AssessResolvers(nq.ResolverSnapshot{Observer: m.Observer, Selections: []nq.ResolverSelection{m.Selection}, AsOf: m.CompletedAt, WindowStart: m.StartedAt, Freshness: time.Second, Measurements: []nq.ResolverMeasurement{m}})
	...
	if c.Result == nq.ResolverAnswer || c.Result == nq.ResolverNXDOMAIN || c.Result == nq.ResolverNoData {
		matched := c.State == nq.StateSucceeded
