...
func New(binding Binding, config Configuration, now time.Time) (Plan, error) {
	...
	prefixes, targetMember, err := checkbinding.Validate(binding.Source, target, binding.Prefixes)
	if err != nil || (config.DestinationScope == EnrolledPrefix && !targetMember) {
		return Plan{}, ErrBinding
	}
...
func ValidateConfiguration(config Configuration) error {
	...
	target := config.Endpoint.Addr()
	if !checkbinding.EligibleHost(target) || (target.Is4() && config.Selection.Family != nq.FamilyIPv4) || (target.Is6() && config.Selection.Family != nq.FamilyIPv6) {
		return ErrConfiguration
	}
	return nil
}
