...
func (c *Control) Run(ctx context.Context, ticket Ticket, consent bool) (Result, error) {
	...
	var measured *nq.HTTPSMeasurement
	...
	// A phase timeout is evidence; it cancels the run only when the enclosing
	// operation or original consent deadline has also expired (checked above).
	if executionErr == context.DeadlineExceeded && measured != nil && measured.Exchange == nq.HTTPSTimeout {
...
func (c *Control) record(ctx context.Context, p pendingReview, state, outcome, reason string, sample *nq.HTTPSMeasurement) error {
	now, err := c.clock()
	if err != nil {
		return err
...
func safeExecute(e Executor, ctx context.Context, s Selection, runID string) (sample nq.HTTPSMeasurement, err error, panicked bool) {
	defer func() {
		if recover() != nil {
	...
	sample, err = e.ExecuteHTTPS(ctx, Request{Selection: s, MeasurementID: runID})
...
func safeAudit(a Auditor, ctx context.Context, e Event) (err error) {
	...
	return a.InsertHTTPSRunAudit(ctx, e)
}
