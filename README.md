...
- A candidate native OPNsense 26.7.4 connection can verify router version over a protected external API link. A separate foreground approval can read bounded ARP/NDP tables once and keep only in-scope router-reported neighbor evidence. This does not establish device identity, whole-network visibility, or router support.
