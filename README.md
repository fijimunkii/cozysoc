...
The [Device Watch storage workload](docs/development/device-watch-storage-workload.md) measured **474.75 MiB** of database growth for 100 stable synthetic neighbors over 1,440 one-minute collections, exceeding the **25 MiB/day** target by about **19×**. [Storage reduction #150](https://github.com/fijimunkii/cozysoc/issues/150) is open. A separate full attribution run measured 474.50 MiB: 58.83% in indexes, with table records alone using 195.363 MiB. Index removal alone cannot meet the target. A [lossless compression experiment](docs/development/device-watch-compression-feasibility.md) identifies a candidate encoding; production persistence and its budget remain unchanged. This accelerated component measurement does not establish CPU/RAM budgets or 24-hour reliability.
