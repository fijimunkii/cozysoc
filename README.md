...
- HTTPS settings and native review disclose the exact request, identity, route, data limits and privacy impact. Internal TLS/HTTP and native TCP candidates produce normalized measurements with response timing. The controller owns one-shot run control with durable audits, cooldown and joined shutdown. The native consent protocol validates the complete disclosure; product HTTPS execution remains disabled pending terminal consent and integrated lab evidence.
